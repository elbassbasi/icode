...
XMLNode::XMLNode(XMLDocument *doc) :
		_document(doc), _parent(0), _value(), _parseLineNum(0), _firstChild(0), _lastChild(
				0), _prev(0), _next(0), _userData(0), _memPool(0) {
}
		...
		if (ele) {
			...
			if (mismatch) {
				_document->SetError(XML_ERROR_MISMATCHED_ELEMENT,
						initialLineNum, "XMLElement name=%s", ele->Name());
				DeleteNode(node);
				break;
			}
		}
...
XMLDocument::XMLDocument(bool processEntities, Whitespace whitespaceMode) :
		XMLNode(0), _writeBOM(false), _processEntities(processEntities), _errorID(
				XML_SUCCESS), _whitespaceMode(whitespaceMode), _errorStr(), _errorLineNum(
				0), _charBuffer(0), _parseCurLineNum(0), _parsingDepth(0), _unlinked(), _elementPool(), _attributePool(), _textPool(), _commentPool() {
...
XMLError XMLDocument::LoadStream(IStream *stream) {
	Clear();
	WResult result = stream->Rewind();
	if (result < 0) {
		SetError(XML_ERROR_FILE_READ_ERROR, 0, 0);
		return _errorID;
	}
	wuint64 filelength;
	result = stream->GetSize(&filelength);
	if (result < 0) {
		SetError(XML_ERROR_FILE_READ_ERROR, 0, 0);
		return _errorID;
	}
	...
	size_t read;
	stream->Read(_charBuffer, size, &read);
	if (read != size) {
		SetError(XML_ERROR_FILE_READ_ERROR, 0, 0);
		return _errorID;
	}
...
XMLPrinter::XMLPrinter(FILE *file, bool compact, int depth) :
		_elementJustOpened(false), _stack(), _firstElement(true), _fp(file), _depth(
